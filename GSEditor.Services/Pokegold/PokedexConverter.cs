...
public sealed class PokedexConverter : IPokegoldConverter
{
  public void Read(PokegoldData data)
  {
    ...
    for (var i = 0; i < 251; i++)
    {
      ...
      item.SpecificName = new(secificNameBytes);
      ...
      item.Description = new(descriptionBytes);
  ...
  public void Write(PokegoldData data)
  {
    ...
    for (var i = 0; i < 251; i++)
    {
      ...
      var specificNameBytes = data.Pokedex[i].SpecificName.ToBytes();
      ...
      var descriptionBytes = data.Pokedex[i].Description.ToBytes();
